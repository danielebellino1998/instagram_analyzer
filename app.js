...
let visitedAccounts = JSON.parse(localStorage.getItem("visitedAccounts")) || [];

function salvaVisitato(username){
 if(!visitedAccounts.includes(username)){
   visitedAccounts.push(username);
   localStorage.setItem(
     "visitedAccounts",
     JSON.stringify(visitedAccounts)
   );
 }
 render();
}
...
function render(){
 ...
 .map(x=>
  ...
  `<div class="item">
    <div>
      <a class="account-link ${visitedAccounts.includes(x) ? 'visited-account' : ''}" target="_blank" href="https://instagram.com/${x}" onclick="salvaVisitato('${x}')">@${x}</a>
     ${

...
function values(obj){
 ...
 function scan(x){
  ...
  if(Array.isArray(x))x.forEach(scan);
   ...
   if(x.title) out.push(x.title);

   if(x.string_list_data) x.string_list_data.forEach(a=>a.value&&out.push(a.value));

   if(x.label_values) x.label_values.forEach(a=>{

     if(a.label==="Nome utente" && a.value) out.push(a.value)
...
async function readFollowing(zip,name){
 ...
 function scan(x){
  ...
  if(Array.isArray(x)) x.forEach(scan);
   ...
   if(x.title){
    ...
    if(x.string_list_data && x.string_list_data[0] && x.string_list_data[0].timestamp){

     followDates[x.title]=new Date(x.string_list_data[0].timestamp*1000).toLocaleDateString("it-IT");
...
async function analizza(){
 ...
 let following=await readFollowing(zip,"following.json");
 ...
 `<div class="stat"><div class="number">${followers.length}</div><div class="label">Followers totali</div></div>

<div class="stat"><div class="number">${following.length}</div><div class="label">Account seguiti</div></div>

<div class="stat"><div class="number">${data.non.length}</div><div class="label">Account che segui ma non ti seguono</div></div>

<div class="stat"><div class="number">${data.fan.length}</div><div class="label">Account che ti seguono ma che non segui</div></div>

<div class="stat"><div class="number">${pending.length}</div><div class="label">Richieste di follow in sospeso</div></div>`;
...
document.getElementById("title").innerHTML="<h2>"+(titles[tipo]||tipo)+"</h2>";
...
function render(){
 ...
 document.getElementById("list").innerHTML=current.filter(x=>x.toLowerCase().includes(q))

 .map(x=>`<div class="item"><div><a target="_blank" href="https://instagram.com/${x}">@${x}</a>${(followDates[x] && data.non.includes(x)) ? `<div style="font-size:12px;color:#94a3b8;margin-top:4px">Segui dal ${followDates[x]}</div>` : ""}</div></div>`).join("");
